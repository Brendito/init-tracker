...
import { getModifier, titleCase } from "../../utils/utils";
import "./styles.css";
...
class StatBlock extends Component<Props> {
  state = {
    spellList: [],
    sticky: true
  };

  //   TODO: Get an idea of what this is doing again
  componentDidMount = () => {
    if (this.props.npc.special_abilities) {
      const abilities = this.props.npc.special_abilities.filter(
  ...
  render() {
    return (
      <div className={!this.props.modal && this.state.sticky ? "sticky" : ""}>
        <div className="d-flex justify-content-start p-2">
          <div
            className={`stat-block ${this.props.modal ? "modalview" : "wide"}`}
          >
            ...
            <div className="section-left">
              <div className="creature-heading">
                ...
                <h2>
                  {this.props.npc.size}{" "}
                  {this.props.npc.type !== "" && titleCase(this.props.npc.type)}{" "}
              ...
              <div className="top-stats">
                ...
                <div className="abilities">
                  <div className="ability-strength">
                    <h4>STR</h4>
                    <p>
                      <span className="d-block">{this.props.npc.strength}</span>{" "}
                      (
                  ...
                  <div className="ability-wisdom">
                    <h4>WIS</h4>
                    <p>
                      <span className="d-block">{this.props.npc.wisdom}</span> (
                  ...
                  <div className="ability-charisma">
                    <h4>CHA</h4>
                    <p>
                      <span className="d-block">{this.props.npc.charisma}</span>{" "}
                      (
                ...
                {this.props.npc.languages && this.props.npc.languages !== "" && (
                  <div className="property-line">
                    <h4>Languages:</h4>
                ...
                <div className="property-line last">
                  <h4>Challenge</h4>
                  <p>
                    {this.props.npc.challenge_rating} - ({this.props.npc.xp}
                    XP)
                  </p>
                </div>
              </div>
              ...
              {this.props.npc.special_abilities &&
                this.props.npc.special_abilities.length > 0 && (
                  <div>
                    ...
                    {this.props.npc.special_abilities.map(
                      (ability: SpecialAbility) => {
                        if (
                          (ability.name === "Spellcasting" ||
                            ability.name === "Innate Spellcasting") &&
                          ...
                          return (
                            <div key={ability.name} className="property-block">
                              ...
                              <ul className="spellList">
                                {this.state.spellList.map((spell: any, i) => {
                                  if (i !== 0) {
                                    return (
                                      <li key={spell}>
              ...
              {this.props.npc.legendary_actions &&
                this.props.npc.legendary_actions.length > 0 && (
                  <div className="actions">
                    ...
                    {this.props.npc.legendary_actions.map((action: Action) => {
                      return (
            ...
            <div className="section-right">
              {this.props.npc.actions && this.props.npc.actions.length > 0 && (
                <div className="actions">
                  <h3>Actions</h3>
                  {this.props.npc.actions.map((action: Action) => {
                    return (
                      <div key={action.name} className="property-block">
                        <h4>{action.name}</h4>
                        <p>
                          ...
                          {action.range && <span>, reach {action.range}.</span>}
                          {(action.average_damage ||
                            action.damage_dice ||
                            action.damage_type) && (
                            <span className="ml-1 mr-1">Hit:</span>
                          )}
                          {action.average_damage && (
                            <span>{action.average_damage}</span>
                          )}
                          {action.damage_dice && (
                          ...
                          {(action.average_damage_bonus ||
                            ...
                            action.bonus_damage_type) && <span> plus </span>}
                          {action.average_damage_bonus && (
                            <span>{action.average_damage_bonus}</span>
                          )}
                          {action.bonus_damage_dice && (
                            <span> ({action.bonus_damage_dice}) </span>
                          )}
                          {action.bonus_damage_type && (
                            <span> {action.bonus_damage_type} </span>
                          )}
                        </p>
                      </div>
                    );
          ...
          {!this.props.modal && (
            <span onClick={this.toggleSticky}>
              <FontAwesomeIcon
                icon="thumbtack"
                className="mt-3"
